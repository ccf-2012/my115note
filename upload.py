...
from p115oss.upload import MultipartUploadAbort
...
def upload_path(path, filename, client: P115Client, pid: int):
    total = None
    ...
    with tqdm(total=total, unit="B", unit_scale=True, desc=f"Uploading {filename}") as t:
        ...
        for i in range(5):
            try:
                resp = client.upload_file(path, **kwargs)
                break
            except MultipartUploadAbort as e:
                kwargs["multipart_resume_data"] = e.ticket
                t.n = 0
                print(f"\n[RETRY] 上传中断，正在尝试断点续传 (尝试 {i+1}/5)...")
        else:
            raise Exception("Upload failed after 5 retries.")
